...
%
% Figures
%   1. fig_diag_fold_rmse.png       — per-fold RMSE bar chart (at optimal r)
%   2. fig_diag_angle_rmse.png      — RMSE per reaching angle (8 directions)
%   3. fig_diag_rmse_over_time.png  — RMSE at each decode timestep
%   4. fig_diag_trajectories.png    — 2x4 grid: red=decoded, blue=actual
% =========================================================================
...
rng(42);
mp = cv_train_model(cv_pool, optimal_r, optimal_M_pca, optimal_M_lda);

%% ---- Figure 1: Per-fold RMSE bar chart ----------------------------------
% Find row in val_rmse_cv corresponding to optimal_r
[~, best_ri] = min(abs(r_grid - optimal_r));
fold_rmse    = val_rmse_cv(best_ri, :);
...
title(sprintf('Per-fold RMSE at optimal r=%d', optimal_r), 'FontSize', 13);
legend('Location', 'best');
grid on;
saveas(fig1, fullfile(script_dir, 'fig_diag_fold_rmse.png'));
...
for d = 1:nDir
    ...
    for tr = 1:nTest
        ...
        if T < t_start; continue; end

        clear positionEstimator; %#ok<CLFUNC>

        startHandPos = hp(1:2, 1);
        t_vec        = t_start : dt_reg : T;
        ...
        for ti = 1:numel(t_vec)
            ...
            test_data.startHandPos = startHandPos;
...
ylabel('RMSE (mm)', 'FontSize', 12);
title('Decoding RMSE per Reaching Direction — LDA\_PCR\_CV (Test Set)', 'FontSize', 13);
xticks(angles_deg);
legend('Location', 'best');
grid on;
saveas(fig2, fullfile(script_dir, 'fig_diag_angle_rmse.png'));
...
ylabel('RMSE (mm)', 'FontSize', 12);
title('Decoding RMSE vs Time into Trial — LDA\_PCR\_CV (Test Set)', 'FontSize', 13);
grid on;
saveas(fig3, fullfile(script_dir, 'fig_diag_rmse_over_time.png'));
...
sgtitle(sprintf('Predicted vs Actual Trajectories — LDA\\_PCR\\_CV (Test Trial %d)', ...
    selected_trial_idx), 'FontSize', 13);
saveas(fig4, fullfile(script_dir, 'fig_diag_trajectories.png'));
...
fprintf('  M_pca  = %d\n', optimal_M_pca);
fprintf('  M_lda  = %d\n', optimal_M_lda);
fprintf('Final test RMSE: %.4f mm\n', final_rmse);
fprintf('Baseline to beat (PCA_RidgeReg): %.2f mm\n', baseline_rmse);
if final_rmse < baseline_rmse
    fprintf('Result: BEATS baseline by %.4f mm\n', baseline_rmse - final_rmse);
else
    fprintf('Result: Does not beat baseline (%.4f mm above)\n', final_rmse - baseline_rmse);
end
fprintf('========================================\n');
