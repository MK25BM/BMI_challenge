...
% cv_evaluate_fold  Evaluate a trained LDA_PCR model on a validation fold.
%
...
% Inputs
% ------
%   val_data        : (nVal x nDir) struct with .spikes and .handPos
...
%   rmse : scalar RMSE in mm
    ...
    t_start = modelParameters.t_reg(1);   % first decode time = 320 ms
    ...
    for d = 1:nDir
        for tr = 1:nVal
            ...
            % Reset persistent state before each trial (safety measure)
            clear positionEstimator; %#ok<CLFUNC>

            % Build startHandPos from first column of handPos
            startHandPos = hp(1:2, 1);

            times = t_start : dt_reg : T;
